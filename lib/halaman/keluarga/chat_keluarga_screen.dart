...
class ChatKeluargaScreen extends StatefulWidget {
  final int perawatId; // ID perawat yang diajak chat
...
class _ChatKeluargaScreenState extends State<ChatKeluargaScreen> 
  ...
  Future<void> _loadMessages({bool loadMore = false}) async {
    try {
      await _chatProvider.loadMessages(widget.perawatId, loadMore: loadMore);
      
      if (!loadMore) {
        // Mark messages as read ketika load pertama kali
        await _chatProvider.markMessagesAsRead(widget.perawatId);
        _scrollToBottom();
      }
    } catch (e) {
      if (mounted) {
  ...
  void _startPolling() {
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 3), (timer) async {
      ...
      try {
        final currentMessageCount = _chatProvider.getMessagesForUser(widget.perawatId).length;
        await _loadMessages();
        final newMessageCount = _chatProvider.getMessagesForUser(widget.perawatId).length;
  ...
  void _scrollListener() {
    ...
    if (_scrollController.offset >= _scrollController.position.maxScrollExtent &&
      ...
      if (_chatProvider.hasMoreMessages(widget.perawatId) && !_isLoadingMore) {
        _loadMoreMessages();
      }
    }
  }
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(context);
              try {
                await _chatProvider.deleteMessage(messageId, widget.perawatId);
              } catch (e) {
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
          ...
          child: Column(
            ...
            children: [
              CircleAvatar(
                ...
                radius: 40,
                child: Text(
                  widget.namaPerawat[0].toUpperCase(),
                ...
                children: [
                    ...
                    children: [
                      ...
                      Consumer<ChatProvider>(
                        builder: (context, provider, child) {
                          return Text(
                            '${provider.getUnreadCountForUser(widget.perawatId)}',
                            style: const TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
                            ),
                  ...
                  Column(
                    children: [
                      ...
                      Consumer<ChatProvider>(
                        builder: (context, provider, child) {
                          final messages = provider.getMessagesForUser(widget.perawatId);
                          return Text(
                            '${messages.length}',
                            style: const TextStyle(
                              fontSize: 18,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(context);
              try {
                await _chatProvider.clearConversation(widget.perawatId);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
  ...
  Widget _buildMessageBubble(dynamic message, bool isMe) {
    ...
    if (message is Map<String, dynamic>) {
      ...
      timestamp = createdAt != null 
          ? DateTime.parse(createdAt).format('HH:mm')
          : '--:--';
    } else {
      // Format baru (ChatMessage)
      messageText = message.message;
      timestamp = message.timeFormatted ?? '--:--';
    }
      ...
      appBar: AppBar(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Consumer<ChatProvider>(
                  builder: (context, provider, child) {
                    final unread = provider.getUnreadCountForUser(widget.perawatId);
                    return Text(
        ...
        actions: [
          ...
          PopupMenuButton(
            ...
            onSelected: (value) {
              if (value == 'mark_read') {
                _chatProvider.markMessagesAsRead(widget.perawatId);
              } else if (value == 'clear') {
                _clearConversation();
              }
            },
              ...
              builder: (context, provider, child) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isMe = message.senderId != widget.perawatId;
